   ...
   console.log("id"+idEntreprise);
    this.entrepriseService.getEnterpriseById(+idEntreprise).subscribe(
      data=>{this.entreprise = data;
        ...
        this.excercie.entreprise=this.entreprise;
  ...
  addExcercice()
{
    ...
    if (e === '0') {
      ...
      this.excerciceService.addExcercice(this.excercie) 
        .subscribe(data => {
          ...
          error => {
            this.content = `Interdiction de créer un exercice ayant la même année qu'un autre existant`;
    ...
    if (e === '1') {
      ...
      .subscribe(
        ...
        this.windowRef.close();
        this.status= NbToastStatus.SUCCESS   ;
        this.title = 'Mise à jour Exercice réussi'
        this.router.navigateByUrl('/', {skipLocationChange: true}).then(()=>
        this.router.navigate(['/pages/exercice']));
        this.makeToast();
        },
        error => {
          console.log('error');
          this.content = `Interdiction de  modifier un exercice ayant la même année qu'un autre existant`;
          this.status= NbToastStatus.DANGER;
          this.makeToast()
        });
    }
